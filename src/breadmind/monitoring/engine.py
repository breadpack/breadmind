...
class LoopProtector:
    ...
    def can_act(self, target: str, action: str) -> bool:
        key = f"{target}:{action}"
        now = datetime.utcnow()
    ...
    def record_action(self, target: str, action: str):
        key = f"{target}:{action}"
        now = datetime.utcnow()
    ...
    async def _run_rule(self, rule: MonitoringRule):
        while self._running:
            try:
                ...
                for event in events:
                    if self._loop_protector.can_act(event.target, event.condition):
                        self._loop_protector.record_action(event.target, event.condition)
                        if self._on_event:
                            await self._on_event(event) if asyncio.iscoroutinefunction(self._on_event) else self._on_event(event)
