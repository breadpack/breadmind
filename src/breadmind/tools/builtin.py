...
def set_bg_job_manager(mgr):
    """Set the BackgroundJobManager reference for DI."""
    global _bg_job_manager
    _bg_job_manager = mgr


@tool(description="Start a long-running background job. Use for tasks that take more than a few minutes (e.g., scanning multiple servers, overnight monitoring). Provide title, job_type ('single' or 'monitor'), steps (list of step descriptions), tools_needed (list of tool names).")
async def run_background(title: str, job_type: str = "single", steps: str = "", tools_needed: str = "", monitor_config: str = ""):
    import json as _json
    if not _bg_job_manager:
        return "Background job system not available. Ensure Redis and Celery are configured."

    # Parse JSON string args from LLM
    step_list = _json.loads(steps) if isinstance(steps, str) and steps.strip().startswith("[") else []
    tool_list = _json.loads(tools_needed) if isinstance(tools_needed, str) and tools_needed.strip().startswith("[") else []
    monitor_cfg = _json.loads(monitor_config) if isinstance(monitor_config, str) and monitor_config.strip().startswith("{") else {}
    ...
    if job_type == "single" and step_list:
        for i, desc in enumerate(step_list):
            ...
            execution_plan.append({"step": i + 1, "description": desc, "tool": t, "args": {}})
    ...
    try:
        result = await _bg_job_manager.create_job(
            title=title,
        ...
        jid = result["job_id"]
        return f"Background job '{title}' started (ID: {jid}). Check progress at /api/bg-jobs/{jid}"
    except ValueError as e:
...
def register_builtin_tools(registry) -> None:
    ...
    for t in [shell_exec, web_search, file_read, file_write, messenger_connect,
              swarm_role, network_scan, router_manage, run_background]:
        registry.register(t)
