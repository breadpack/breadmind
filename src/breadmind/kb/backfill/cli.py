...
def _add_confluence_subparser(sub: argparse._SubParsersAction) -> None:  # noqa: SLF001
...
def format_confluence_dry_run(report: JobReport, ctx: dict) -> str:
    ...
    org_label = ctx.get("org_label", str(report.org_id))
    ...
    all_skip_keys = sorted(report.skipped.keys())
    ...
    if report.sample_titles:
        lines += ["", "Sample skips:"]
        for title in report.sample_titles[:3]:
            lines.append(f"  {title}")

    lines += [
        "",
        "Run again without --dry-run to commit.",
        f"JobReport id: {report.job_id}",
    ]
    return "\n".join(lines)
...
async def _run_confluence(
    ...
    credentials_ref = f"confluence:org:{args.org}"
    ...
    if getattr(args, "resume", None):
        job._resume_cursor = args.resume
