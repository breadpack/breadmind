...
def setup_swarm_routes(r: APIRouter, app_state):
    ...
    async def swarm_roles(swarm_manager=Depends(get_swarm_manager)):
        if not swarm_manager:
            return {"roles": []}
        return {"roles": swarm_manager.get_available_roles()}

    @r.post("/api/swarm/roles")
    async def add_swarm_role(
        request: Request,
        swarm_manager=Depends(get_swarm_manager),
    ...
    async def delete_swarm_role(
        ...
        if not swarm_manager:
            return JSONResponse(status_code=503, content={"error": "Swarm manager not configured"})
        removed = swarm_manager.remove_role(role_name)
        if not removed:
