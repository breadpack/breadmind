...
1. OS/패키지 매니저 감지
   └─ Linux (apt/dnf/pacman) 또는 macOS (brew)

2. Python 3.12+ 확인
   ├─ 있으면 → 사용
   └─ 없으면 → 패키지 매니저로 자동 설치 (확인 후)

3. PostgreSQL 포트 확인 (5432)
   ├─ 이미 PostgreSQL이 있으면 → "기존 DB 사용할까요?" 질문
   ├─ 다른 서비스가 5432 사용 → 5433~5440에서 빈 포트 탐색
   └─ 비어있으면 → 5432 사용

4. Docker 확인 (PostgreSQL 컨테이너용)
   ├─ 있으면 → 사용
   ├─ 없으면 → "Docker 설치할까요?" 질문
   └─ 거부하면 → --external-db 모드 전환 (수동 DB 구성)

5. BreadMind 설치
   ├─ pip install breadmind (PyPI) 시도
   └─ 실패 시 → pip install git+https://github.com/breadpack/breadmind.git

6. 설정 파일 생성
   ├─ ~/.config/breadmind/config.yaml (LLM, DB, MCP 설정)
   ├─ ~/.config/breadmind/safety.yaml (도구 블랙리스트)
   └─ ~/.config/breadmind/.env (API 키, DB 비밀번호)
       └─ 대화형 모드: Anthropic API 키 입력 프롬프트

7. PostgreSQL Docker 컨테이너 시작
   └─ pgvector/pgvector:pg17 이미지
       ├─ DB: breadmind
       ├─ User: breadmind
...
```bash
# 기본 설치 (Docker로 PostgreSQL 자동 구성)
curl -fsSL .../install.sh | bash

# 기존 PostgreSQL 사용 (Docker 불필요)
curl -fsSL .../install.sh | bash -s -- --external-db
# → DB 연결 정보를 ~/.config/breadmind/config.yaml에서 수동 설정

# 비대화형 모드 (파이프 실행 시 자동)
# → 모든 질문에 기본값(Yes) 사용, API 키 프롬프트 건너뜀
```
...
```
~/.config/breadmind/
├── config.yaml          # LLM, DB, MCP 설정
├── safety.yaml          # 도구 블랙리스트/승인 목록
└── .env                 # API 키, DB 비밀번호

Docker 컨테이너:
└── breadmind-postgres   # PostgreSQL + pgvector (포트 5432)
